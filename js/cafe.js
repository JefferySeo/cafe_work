$(function(){
   ...
   $('.search-select').click(function(e){
      ...
      if(dp == "none"){
         $('.selectbox').find('.fa-solid').removeClass('fa-angle-down').addClass('fa-angle-up');//none이라면 아래 화살표를 지우고 위에 화살표를 넣는다.
   ...
   jQuery.ajax({
      ...
      success: function(data){
         ...
         for(let i = 0; i < data.cafelist.length ; i++){
            ...
            list += '<div class="cfe d-flex"><p class="ellipise">'+data.cafelist[i].cafename +'</p><p class="dg">'+data.cafelist[i].comment+'</p></div></a></li>';
         }
         $('.clist').html(list);
      },
...
function mySlide(){
   ...
   // 제일 앞에 보이게 하고 투명처리 한 후 점차 진하게 보이게 한다.
   eq1.addClass('zindex').css('opacity',0).animate({
      'opacity':1
   },500,function(){
      // animate 작업이 끝나면 이전에 보였던 new의 zindex를 지우고
      // 가장 나중으로 바꿔준다. 
